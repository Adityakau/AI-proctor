...
import com.example.proctoring.ingest.repository.AlertRepository;
import com.example.proctoring.ingest.repository.AnomalyEventRepository;
...
public class EventIngestService {
        ...
        if (request.getThumbnails() != null) {
            for (var thumb : request.getThumbnails()) {
                try {
                    ...
                    java.nio.file.Path path = java.nio.file.Paths.get("/tmp/proctoring/thumbnails");
                    ...
                    if (event != null) {
                        event.setThumbnailMetaJson(objectMapper.writeValueAsString(Map.of(
                                "path", path.resolve(filename).toString(),
                                "size", thumb.getSizeBytes())));
    ...
    private void generateAlertIfNeeded(String sessionId, AnomalyEventDTO event) {
        ...
        // Generate alerts for HIGH/CRITICAL severity or specific event types
        ...
        if ("CRITICAL".equals(severity) || "HIGH".equals(severity)) {
            ...
            // Check recent event count in Redis for escalation
