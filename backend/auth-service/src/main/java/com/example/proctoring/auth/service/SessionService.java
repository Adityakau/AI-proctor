package com.example.proctoring.auth.service;

import com.example.proctoring.auth.repository.ProctoringSessionRepository;
...
public class SessionService {
    ...
    public SessionService(ProctoringSessionRepository repository) {
    ...
    public ProctoringSession heartbeat(JwtClaims claims) {
        ProctoringSession session = getExistingSession(claims);
        session.setLastHeartbeatAt(Instant.now());
    ...
    private ProctoringSession getExistingSession(JwtClaims claims) {
        return repository
                .findByTenantIdAndExamScheduleIdAndUserIdAndAttemptNo(
                        claims.getTenantId(),
                        claims.getExamScheduleId(),
