...
const CONFIG = {
  ...
  THUMBNAIL_MIN_INTERVAL_MS: 10000, // 10 seconds for HIGH/CRITICAL events
  THUMBNAIL_MAX_SIZE_KB: 50, // Increased for better quality
  THUMBNAIL_WIDTH: 640, // Higher resolution for dashboard/ML
...
export default function ProctoringPage() {
  ...
  const trackingRef = useRef({
    ...
    pendingEvents: [],
    // SPOOF_SUSPECTED: frozen frame detection
    lastFrameData: null,
    frozenFrameSince: null,
  });
  ...
  const runDetection = useCallback(() => {
    ...
    try {
      ...
      // SPOOF_SUSPECTED (frozen frame detection)
      const currentFrameData = JSON.stringify({ brightness: Math.round(brightness), variance: Math.round(variance) });
      if (tracking.lastFrameData && tracking.lastFrameData === currentFrameData) {
        // Frame appears frozen (very similar brightness/variance)
        if (!tracking.frozenFrameSince) tracking.frozenFrameSince = now;
        if (now - tracking.frozenFrameSince > 10000) { // 10 seconds of frozen frame
          addEvent("SPOOF_SUSPECTED", "HIGH", 0.8, {
            reason: "frozen_frame",
            durationMs: now - tracking.frozenFrameSince,
          }, true);
          tracking.frozenFrameSince = now; // Reset to avoid spam
        }
      } else {
        tracking.frozenFrameSince = null;
      }
      tracking.lastFrameData = currentFrameData;
    } catch (e) {
  ...
  }, [status, sendBatch, pollAlerts, sendHeartbeat]);
