...
echo ""
echo "[5/5] Testing idempotency (resend same event)..."
IDEMPOTENCY_RESPONSE=$(curl -s -X POST "$API_BASE/proctoring/events/batch" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{
    "sessionId": "'$SESSION_ID'",
    "events": [
      {"eventId": "evt-test-1-idempotency", "type": "LOOK_AWAY", "timestamp": "'$TIMESTAMP'", "confidence": 0.85, "severity": "MEDIUM", "details": {}}
    ],
    "thumbnails": []
  }')

FIRST_ACCEPTED=$(echo $IDEMPOTENCY_RESPONSE | grep -o '"acceptedEventIds":\[[^]]*\]')
echo "First send: $FIRST_ACCEPTED"

# Resend same event
IDEMPOTENCY_RESPONSE2=$(curl -s -X POST "$API_BASE/proctoring/events/batch" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{
    "sessionId": "'$SESSION_ID'",
...
REJECTED=$(echo $IDEMPOTENCY_RESPONSE2 | grep -o '"rejectedEventIds":\[[^]]*\]')
if echo "$REJECTED" | grep -q "evt-test-1-idempotency"; then
  echo "✅ Idempotency verified: duplicate event rejected"
else
  echo "⚠️ Idempotency check: $IDEMPOTENCY_RESPONSE2"
fi
